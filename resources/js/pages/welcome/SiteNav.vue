    ...
    <header class="fixed inset-x-0 top-0 z-50 border-b transition-colors duration-300" :class="scrolled
        ...
        <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            <a href="#top" class="group flex items-center gap-2">
                ...
                <span class="font-display text-lg font-medium tracking-tight text-foreground">
            ...
            <div class="hidden items-center gap-2 md:flex">
                <Link :href="login.url()"
                    class="px-3 py-2 text-sm text-foreground/80 transition-colors hover:text-foreground">
                    Log in
                </Link>
                <Link v-if="canRegister" :href="register.url()">
                    <Button size="sm" class="bg-foreground font-medium text-background hover:bg-foreground/90">
                        Get started
                    </Button>
                </Link>
            </div>

            <button type="button" class="grid size-9 place-items-center text-foreground hover:bg-secondary md:hidden"
        ...
        <div v-if="mobileOpen" class="border-t border-border bg-background px-4 py-4 md:hidden">
            <nav class="flex flex-col gap-1">
                <a v-for="link in navLinks" :key="link.href" :href="link.href"
                    class="px-3 py-2.5 text-sm text-foreground hover:bg-secondary" @click="mobileOpen = false">
                    {{ link.label }}
                </a>
                <div class="mt-2 grid grid-cols-2 gap-2 border-t border-border pt-3">
                    <Link :href="login.url()"
                        class="border border-border px-3 py-2 text-center text-sm text-foreground hover:bg-secondary">
                        Log in
                    </Link>
                    <Link v-if="canRegister" :href="register.url()"
                        class="bg-foreground px-3 py-2 text-center text-sm font-medium text-background">
                        Get started
                    </Link>
                </div>
            </nav>
